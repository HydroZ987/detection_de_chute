...
if not api_key:
    print("ATTENTION: Variable d'environnement OPENAI_API_KEY non définie")
...
def summarize_fall_scene(frames):
    ...
    QUESTION = ("Décris précisément cette scène de vidéo-surveillance. "
                "Concentre-toi sur la personne qui est tombée. "
                "Y a-t-il un danger ou un besoin d’intervention ? Réponds en français.")

    response = client_ai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "Tu es un agent de sécurité vidéo intelligent."},
            {"role": "user", "content": [
                "Voici les images extraites d'une chute.",
...
modesave = 1
dir_videos = r"C:\Users\lemoi\Downloads\py_env\py_env\output"
fichier_video = None
...
historique_frames = []
MAX_HIST_FRAMES = 15  # stocker les 15 dernières frames (~1 seconde si 15fps)
...
while cap.isOpened():
    ...
    # Vérifier que la frame a été lue correctement
    if not ret or frame is None:
        break
    
    frame = resizeFrame(frame, desired_height=desired_height) if (frame.shape[0]!=desired_height) else frame

    # Sauvegarder les frames pour résumé LLM
    if len(historique_frames) >= MAX_HIST_FRAMES:
        historique_frames.pop(0)
    historique_frames.append(frame.copy())
        ...
        if modesave:
            ...
            if fichier_video is not None:
                ...
                if cpt_fin_mouvement == 0:
                    ...
                    # sending message
                    try:
                        # Résumer la scène à partir des dernières frames
                        ...
                        #message = client.messages.create(
                            ...
                            #to="+33620023990"
                        #)
                        print("Résumé envoyé par SMS.")
                    except Exception as e:
                        print("Erreur lors de la génération du résumé ou de l'envoi :", e)
